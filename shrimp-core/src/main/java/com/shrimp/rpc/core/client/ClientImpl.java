...
import com.shrimp.rpc.core.exception.RequestTimeoutException;
...
public class ClientImpl implements Client {
    ...
    public void init() {
        ...
        try {
            ...
            //通过curator API的PathCache用来监控一个ZNode节点
            ...
            pathChildrenCache.getListenable().addListener((client, event) -> {
                ...
                //关闭删除本地缓存中多出的channel
                for (ChannelConf channelConf : channelWrappers) {
                    String connStr = channelConf.getConnStr();
                    if (!newServiceData.contains(connStr)) {
                        channelConf.close();
                        LOGGER.info("Remove channel {}", connStr);
                        channelWrappers.remove(channelConf);
                    }
                }
                ...
                for (String connStr : newServiceData) {
                    ...
                    for (ChannelConf cw : channelWrappers) {
                        if (connStr.equals(cw.getConnStr())) {
                            containThis = true;
                        }
                    }
                    if (!containThis) {
    ...
    private void addNewChannel(String connStr) {
        try {
            ...
            ChannelConf channelWrapper = new ChannelConf(ip, port);
    ...
    public Response sendMessage(Class<?> clazz, Method method, Object[] args) {
        ...
        ChannelConf channelWrapper = setChannel();
        ...
        //当channel的配置连接不为空时， 就可以取到channelle
    ...
    private ChannelConf setChannel() {
